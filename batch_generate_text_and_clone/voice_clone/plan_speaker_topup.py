...
def task_for(
    ...
    ref_index = ordinal % len(references)
    cycle = ordinal // len(references)
    ...
    task_id = stable_hash(
        "speaker-topup-v1", round_id, speaker_key, ordinal, reference["utt_id"],
        text["id"], text["text"],
    )
...
def write_csv_atomic(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    fields = [
        ...
        "deficit_seconds", "planned_tasks", "reference_count",
    ]
    try:
...
def main() -> int:
    ...
    accepted, accepted_counts, _ = inventory_accepted(
    ...
    for speaker_key in sorted(target_paths):
        ...
        task_count = 0
        if deficit:
            task_count = math.ceil(deficit / args.planning_seconds_per_clone) + args.reserve_tasks
            ...
            task_rows.extend(
                task_for(
                    args.round_id, speaker_key, ordinal, refs, texts,
                    baseline, args.target_seconds,
                )
